...
export function AdminDashboardV2({ section }: { section: AdminSection }) {
  ...
  async function handleRestartEsp32Enrollment(employeeId: string, deviceCode?: string | null) {
    setSubmitting(true);
    setMessage(null);
          ...
          <section className="grid">
            <form className="panel span-2" onSubmit={handleCreateEmployee}>
              ...
              <label className="field">
                <span>Тип створення</span>
                <select
                  value={createEmployee.mode}
                  onChange={(e) =>
                    setCreateEmployee((current) => ({
                      ...current,
                      mode: e.target.value as "standard" | "terminal_only",
                    }))
                  }
                >
                  <option value="terminal_only">ESP32 + RFID + відбиток</option>
                  <option value="standard">Звичайний працівник з email і паролем</option>
                </select>
              </label>
              <label className="field">
                <span>ПІБ</span>
              ...
              <button className="button button-primary full-width" disabled={submitting} type="submit">
                {createEmployee.mode === "terminal_only" ? "Створити і відправити на enroll" : "Створити"}
              </button>
            </form>
          </section>
                ...
                {employees
                  ...
                  .map((employee) => (
                    <EditableEmployeeCardV2
                      ...
                      onRestartEnrollment={handleRestartEsp32Enrollment}
                    />
                ))}
              </div>
            </article>
...
function EditableEmployeeCardV2({
  ...
  onRestartEnrollment: (employeeId: string, deviceCode?: string | null) => Promise<void>;
}) {
        ...
        <div className="editor-card-head">
          <div className="editor-card-title">
            ...
            <span className="hint-text">
              {terminalProfile === "esp32_rfid" ? "ESP32 + RFID + відбиток" : email}
            </span>
            <div className="employee-access-badges">
              ...
              <span className={rfidUid ? "status-paid" : "status-unpaid"}>
                {rfidUid ? `RFID: ${rfidUid}` : "RFID не задано"}
              </span>
          ...
          <div className="editor-card-actions">
            ...
            {terminalProfile === "esp32_rfid" ? (
              <button
                ...
                type="button"
                disabled={disabled}
                onClick={() => onRestartEnrollment(employee.id, settings?.enrollment_device_code ?? null)}
              >
                Запустити enroll
              </button>
            ) : null}
          ...
          <>
            ...
            <div className="field-row">
              ...
              <label className="field">
                ...
                <select value={terminalProfile} onChange={(e) => setTerminalProfile(e.target.value as "raspberry_pi" | "esp32_rfid")}>
        ...
        {securityOpen ? (
          <>
            ...
            <div className="panel popover employee-security-modal employee-security-popover">
              ...
              <div className="hint-text">
                Поточний пароль не можна переглянути. Тут можна лише задати новий. Для ESP32-профілю можна або задати PIN / RFID / Fingerprint вручну, або натиснути `Запустити enroll` і пройти весь сценарій на терміналі.
              </div>
